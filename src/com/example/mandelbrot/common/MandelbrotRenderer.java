...
public final class MandelbrotRenderer {
    ...
    private static int mandelbrotColor(double cx, double cy, int maxIter) {
        ...
        // Внутри множества — чёрный.
        if (iter >= maxIter) return 0x000000;

        // Плавный цвет: t = iter + 1 - log(log(|z|)) / log(2).
        ...
        // hue зависит от t (циклим).
        double hue = (t / 32.0) % 1.0;       // 32 — «скорость» смены палитры
        double saturation = 0.85;
        // value зависит от sqrt(t/maxIter) — мягче и ярче на границе.
